...
\mathbb{O}_D := \mathbb{R}^D \setminus \{0\}.
\]
Fix \(q_{\mathrm{amb}} \in [1,\infty]\). For each \(x \in \mathbb{O}_D\), write
\[
x = \rho\,\theta,
\qquad
\rho := \|x\|_{q_{\mathrm{amb}}} > 0,
\qquad
\theta := \frac{x}{\|x\|_{q_{\mathrm{amb}}}} \in \mathbb{S}^{D-1}_{q_{\mathrm{amb}}},
\]
where
\[
\mathbb{S}^{D-1}_{q_{\mathrm{amb}}} := \{u \in \mathbb{R}^D : \|u\|_{q_{\mathrm{amb}}} = 1\}.
\]
...
\begin{enumerate}
    ...
    \[
    g : \mathbb{S}^{D-1}_{q_{\mathrm{amb}}} \to \mathbb{R}^m
    \]
    such that
    \[
    f(x) = \|x\|_{q_{\mathrm{amb}}}^p\, g\!\left(\frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right),
...
f(\lambda x)
=
\|\lambda x\|_{q_{\mathrm{amb}}}^p\, g\!\left(\frac{\lambda x}{\|\lambda x\|_{q_{\mathrm{amb}}}}\right)
=
(\lambda \|x\|_{q_{\mathrm{amb}}})^p\, g\!\left(\frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right)
=
\lambda^p f(x),
\]
since \(\|\lambda x\|_{q_{\mathrm{amb}}} = \lambda \|x\|_{q_{\mathrm{amb}}}\) for \(\lambda > 0\). Thus \(f\) is positively homogeneous of degree \(p\).
...
\[
g(\theta) := f(\theta),
\qquad \theta \in \mathbb{S}^{D-1}_{q_{\mathrm{amb}}}.
\]
For any \(x \in \mathbb{O}_D\), we have
\[
x = \|x\|_{q_{\mathrm{amb}}} \frac{x}{\|x\|_{q_{\mathrm{amb}}}},
\]
and therefore, by positive homogeneity,
\[
f(x)
=
f\!\left(\|x\|_{q_{\mathrm{amb}}} \frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right)
=
\|x\|_{q_{\mathrm{amb}}}^p\, f\!\left(\frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right)
=
\|x\|_{q_{\mathrm{amb}}}^p\, g\!\left(\frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right).
\]
This proves existence.

To prove uniqueness, suppose also that
\[
f(x) = \|x\|_{q_{\mathrm{amb}}}^p\, \tilde g\!\left(\frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right)
\]
for some map \(\tilde g : \mathbb{S}^{D-1}_{q_{\mathrm{amb}}} \to \mathbb{R}^m\). Let
\(\theta \in \mathbb{S}^{D-1}_{q_{\mathrm{amb}}}\). Since \(\|\theta\|_{q_{\mathrm{amb}}} = 1\), evaluating both
...
g(\theta)=\tilde g(\theta)
\qquad
\text{for every } \theta \in \mathbb{S}^{D-1}_{q_{\mathrm{amb}}}.
\]
Hence \(g=\tilde g\).
\end{proof}
...
\(\|\cdot\|_{q_{\mathrm{amb}}}\), a positively homogeneous map is completely determined by its
restriction to the \(q_{\mathrm{amb}}\)-sphere: the scaling behaviour is prescribed by \(\|x\|_{q_{\mathrm{amb}}}^p\), and all remaining structure is represented by the angular component \(g\).
\end{remark}

A corollary of \ref{prop:canonical-homogeneous-form} is that, on the subset
\[
\{\theta \in \mathbb{S}^{D-1}_{q_{\mathrm{amb}}} : g(\theta) \neq 0\},
\]
one may further decompose \(g\) into magnitude and direction:
\[
g(\theta) = a(\theta)e(\theta),
\qquad
a(\theta) := \|g(\theta)\|_{q_{\mathrm{lat}}},
\qquad
e(\theta) := \frac{g(\theta)}{\|g(\theta)\|_{q_{\mathrm{lat}}}} \in \mathbb{S}^{m-1}_{q_{\mathrm{lat}}},
\]
where \(q_{\mathrm{lat}} \in [1,\infty]\) is a latent-space norm exponent.

Thus, for every \(x \in \mathbb{O}_D\) such that
\(g(x/\|x\|_{q_{\mathrm{amb}}}) \neq 0\), we always have the decomposition
\[
f(x)
=
\|x\|_{q_{\mathrm{amb}}}^p\,
a\!\left(\frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right)
e\!\left(\frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right).
\]

Under this representation, \(a\) can be considered as an angle-dependent scaling factor, \(e\) as an angle-map, and \(\|x\|_{q_{\mathrm{amb}}}^p\) as a purely radial scaling term.

More generally, the same argument applies whenever one replaces
\(\|x\|_{q_{\mathrm{amb}}}\) by a positive radial function \(r : \mathbb{O}_D \to (0,\infty)\)
satisfying
\[
...
\qquad x\in\mathbb{O}_D,
\]
for a unique angular map \(g:\mathbb{S}^{D-1}_{q_{\mathrm{amb}}}\to\mathbb{R}^m\). We therefore need only parameterise the angular component of the encoder.
...
e(\theta):=\frac{g(\theta)}{\|g(\theta)\|_{q_{\mathrm{lat}}}}\in\mathbb{S}^{m-1}_{q_{\mathrm{lat}}},
\]
where \(q_{\mathrm{lat}}\in[1,\infty]\) is a latent-space norm exponent. The encoder then takes the form
\[
f(x)
=
\|x\|_{q_{\mathrm{amb}}}^p\,
...
e\!\left(\frac{x}{\|x\|_{q_{\mathrm{amb}}}}\right).
\]
Here \(\|x\|_{q_{\mathrm{amb}}}^p\) is the homogeneous radial term, \(a\) is an angle-dependent scaling, and \(e\) is an angular map from the ambient sphere to the latent sphere. This secondary decomposition, whilst theoretically serving only a limited role, is helpful for practical model training purposes such as diagnostics, regularisation and visualisation.

For implementation, \(a\) and \(e\) are represented by neural networks on \(\mathbb{S}^{D-1}_{q_{\mathrm{amb}}}\):
\[
a(\theta)=\mathrm{softplus}\!\bigl(A_\phi(\theta)\bigr),
\qquad
e(\theta)=\frac{E_\phi(\theta)}{\|E_\phi(\theta)\|_{q_{\mathrm{lat}}}},
\]
where
\[
A_\phi:\mathbb{S}^{D-1}_{q_{\mathrm{amb}}}\to\mathbb{R},
\qquad
E_\phi:\mathbb{S}^{D-1}_{q_{\mathrm{amb}}}\to\mathbb{R}^m.
\]
We assume that \(E_\phi(\theta)\neq 0\) for all \(\theta\) in the domain of interest, so that the normalised direction map is well-defined. The softplus nonlinearity enforces positivity of \(a\), and normalisation enforces \(e(\theta)\in\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\). By construction, the resulting encoder is exactly positively homogeneous of degree \(p\). The importance of this formulation is that homogeneity of the encoder is not imposed by penalty or approximation but is ensured via the architectural setup. Then, by \Cref{prop:reg-variation-under-homogeneous-encoders}, the latent representation
\[
Z=f(X)
\]
...
z=\rho\,\eta,
\qquad
\rho:=\|z\|_{q_{\mathrm{lat}}},
\qquad
\eta:=\frac{z}{\|z\|_{q_{\mathrm{lat}}}}\in\mathbb{S}^{m-1}_{q_{\mathrm{lat}}},
\]
...
\qquad
\tilde e:\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\to \mathbb{S}^{D-1}_{q_{\mathrm{amb}}},
\qquad
\tilde a:\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\to (0,\infty),
\]
with angular map \(\tilde e\) and decoder-side magnitude term \(\tilde a\). This is the natural inverse analogue of the encoder parameterisation \(f(x)=\|x\|_{q_{\mathrm{amb}}}^p\,a(\theta)\,e(\theta)\).
...
\[
\delta:\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\times(0,\infty)\to \mathbb{R}^D
\]
...
{\bigl\|\tilde e(\eta)+\delta(\eta,\rho)\bigr\|_{q_{\mathrm{amb}}}}
\in \mathbb{S}^{D-1}_{q_{\mathrm{amb}}},
\]
...
\[
\sup_{\eta\in\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}}\|\delta(\eta,\rho)\|\to 0
\qquad\text{as }\rho\to\infty,
\]
together with a nondegeneracy condition ensuring that
\[
\inf_{\eta,\rho}\bigl\|\tilde e(\eta)+\delta(\eta,\rho)\bigr\|_{q_{\mathrm{amb}}}>0.
\]
Under these assumptions,
\[
\sup_{\eta\in\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}}
...
\[
\Delta_\psi:\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\times[0,1]\to \mathbb{R}^D
\]
...
\]
Since \(s(\rho)\to 0\) as \(\rho\to\infty\) and \(\Delta_\psi\) is continuous on the compact set \(\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\times[0,1]\), by the uniform continuity theorem we have
\[
\sup_{\eta\in\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}}\|\delta(\eta,\rho)\|\to 0
\qquad\text{as }\rho\to\infty.
\]
...
\qquad
\tilde e(\eta)=\frac{E_\psi(\eta)}{\|E_\psi(\eta)\|_{q_{\mathrm{amb}}}},
\qquad
\delta(\eta,\rho)=\Delta_\psi\bigl(\eta,s(\rho)\bigr)-\Delta_\psi(\eta,0),
\]
with neural networks
\[
A_\psi:\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\to \mathbb{R},
\qquad
E_\psi:\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\to \mathbb{R}^D,
\qquad
\Delta_\psi:\mathbb{S}^{m-1}_{q_{\mathrm{lat}}}\times[0,1]\to \mathbb{R}^D.
\]
