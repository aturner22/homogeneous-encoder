...
\lambda>0,
\]
for all measurable \(A \subset \mathbb{O}_D\). See also \cite[Theorem 6.1 and Section 6.5.5]{resnick_heavy-tail_2007}. For avoidance of confusion, note that here and throughout, homogeneity of \emph{measures} refers to scaling of dilated sets: $\mu(\lambda A)=\lambda^{-\alpha}\mu(A)$ and homogeneity of \emph{maps} refers to the usual scaling relation $f(\lambda x)= \lambda^p f(x)$.

[TODO: CHECK THIS - It's a bit of a claim]
When \(X\) is supported on a manifold \(\cM\) and is regularly varying with homogeneous tail measure \(\mu\), the homogeneity of \(\mu\) implies that its support is necessarily a cone (invariant under positive dilations). This means that extreme observations on \(\cM\) asymptotically concentrate on directions described by the conical set \(\mathrm{supp}(\mu)\). While \(\cM\) may be curved at moderate scales, its tail behavior is governed by conical geometry, which motivates our use of asymptotically homogeneous decoders.
...
We aim to use observations to learn a latent representation of \(X\) in a lower-dimensional Euclidean\footnote{In this work, the data manifold is assumed to be diffeomorphic to $\RR^m$. Extending the presented framework to other manifold topologies would be a natural direction for further research.} space while preserving its tail asymptotics. We introduce
...
\]
as the \emph{decoder}.
The latent representation of $X$ is
\[
Z := f(X).
\]

To recap, \(\mathcal{M}\subset\mathbb{R}^D\) is an ambient embedded manifold and the latent variable \(Z\) takes values in \(\mathbb{R}^m\) serving as an intrinsic coordinate space. The decoder \(h\) may be used to reconstruct ambient observations from latent coordinates.

Classical dimension-reduction and representation-learning methods, including principal component methods [TODO: CITE], nonlinear embeddings [TODO: CITE], and generic autoencoder architectures [TODO: CITE], generally provide no structural guarantee that regular variation is preserved under the learned representation map [TODO: SHOW THIS/CASES IN WHICH THIS CLAIM IS TRUE/FALSE]. In particular, for a generic [TODO: NONLINEAR?] transformation \(f\), there is typically no reason for the latent law of \(f(X)\) to remain regularly varying [TODO: EXPAND ON THIS?], nor for its tail measure to be related in a controlled way to the ambient tail measure \(\mu\) [TODO: EXPAND ON THIS?]. With the objective of conducting extreme value analysis in latent space, the lack of preservation of tail properties under arbitrary transformation is a fundamental barrier to principled modelling of heavy-tailed data possessing high ambient dimension but a lower intrinsic dimension.

The core contribution of this paper is to present the idea to restrict the encoder to a class of maps explicitly compatible with regular variation. More precisely, we seek encoders \(f\) such that the tail measure of the latent representation exists and is given by the pushforward of the ambient tail measure:
\[
\mu_f := f_{\#}\mu = \mu \circ f^{-1}.
\]
with \(f_{\#}\mu\) denoting the pushforward of \(\mu\) through \(f\). Then, the latent representation preserves the tail asymptotics of the data whenever \(f(X)\) remains regularly varying with nonzero tail measure \(f_{\#}\mu\) [TODO: GO INTO MORE DETAIL HERE/SHOW THIS].
...
Let \(h : (\mathbb{O}, \mathscr{S}_{\mathbb{O}}) \to (\mathbb{O}', \mathscr{S}_{\mathbb{O}'})\) be a measurable mapping such that \(h^{-1}(A')\) is bounded away from \(\mathbb{C}\) for every \(A' \in \mathscr{S}_{\mathbb{O}'} \cap h(\mathbb{O})\) that is bounded away from \(\mathbb{C}'\). Then the pushforward map
\[
\widehat{h}:\mathbb{M}_{\mathbb{O}} \to \mathbb{M}_{\mathbb{O}'},
\qquad
\widehat{h}(\nu)=\nu\circ h^{-1},
\]
is continuous at \(\mu\) provided \(\mu(D_h)=0\), where \(D_h\) denotes the set of discontinuity points of \(h\).
\end{theorem}
...
t\,\mathbb{P}\!\left(\frac{X}{b(t)}\in \cdot\right).
\]
By assumption that $X$ is regularly varying, \(\nu_t \to \mu\) in \(\mathbb{M}(\mathbb{O}_D)\). Since \(f\) satisfies the hypotheses of the mapping theorem and since \(f\) is continuous \(\mu\)-almost everywhere: \(\mu(D_f)=0\), it follows that
\[
\nu_t \circ f^{-1}
\;\xrightarrow[t\to\infty]{}\;
\mu\circ f^{-1}
...
This motivates the core architectural choice of this paper: structurally enforcing homogeneity of the encoder so that tail behaviour is transported to latent space in closed form. In contrast, the decoder need not be globallyw homogeneous and in fact, this ought to be avoided altogether [TODO: MAYBE EXPLAIN THIS MORE USING DIAGRAMS OF HOW HOMOGENEOUS ENCODER CAN CAUSE OFF-MANIFOLD RECONSTRUCTIONS]. Exact homogeneity is useful for tail transport, but in fact structurally incompatible for reconstruction of curved manifold geometry. Instead, we require only that the decoder is \emph{asymptotically homogeneous}, meaning that it converges to a homogeneous map at large scales while retaining flexibility at finite scales to capture manifold curvature.

To achieve this, the decoder is designed to be homogeneous at large scales but admits non-homogeneous corrections at moderate scales to capture curved features of \(\cM\). A these corrections are regularised to vanish in the tail, ensuring that at large scales, decoding is controlled by a homogeneous component, so the asymptotic properties of the latent representation are retained upon reconstruction. [TODO: REWRITE THIS SECTION TO EXPLAIN WHY ASYMPTOTIC HOMOGENEITY IS COMPATIBLE WITH THE AMBIENT MANIFOLD BY ASSUMPTION]. This is the basis of the presented autoencoder framework: latent representations are learned in a way that is compatible with regular variation, while complex ambient manifold geometry can be recovered through an adaptively asymptotically homogeneous decoder.
...
for a given norm. This is extremely useful in a machine learning setting: since homogeneity is enforced by construction in this representation, one may parameterise the angular component unrestricted while always satisfying the required scaling law. This observation motivates the encoder architecture introduced later in
Section~\ref{sec:encoder-architecture} [TODO: REF SECTION].
...
\rho := \|x\|_q > 0,
\qquad
\theta := \frac{x}{\|x\|_q} \in \mathbb{S}^{D-1}_q,
\]
where
\[
\mathbb{S}^{D-1}_q := \{u \in \mathbb{R}^D : \|u\|_q = 1\}.
\]
...
\begin{enumerate}
    ...
    \[
    g : \mathbb{S}^{D-1}_q \to \mathbb{R}^m
    \]
    such that
    \[
...
\begin{proof}
Firstly, to verifying that the provided factorisation is indeed homogeneous, suppose that
\[
f(x) = \|x\|_q^p\, g\!\left(\frac{x}{\|x\|_q}\right),
\qquad x \in \mathbb{O}_D.
\]
...
f(\lambda x)
=
\|\lambda x\|_q^p\, g\!\left(\frac{\lambda x}{\|\lambda x\|_q}\right)
=
(\lambda \|x\|_q)^p\, g\!\left(\frac{x}{\|x\|_q}\right)
=
\lambda^p f(x),
\]
since \(\|\lambda x\|_q = \lambda \|x\|_q\) for \(\lambda > 0\). Thus \(f\) is
...
f(x)
=
f\!\left(\|x\|_q \frac{x}{\|x\|_q}\right)
=
\|x\|_q^p\, f\!\left(\frac{x}{\|x\|_q}\right)
=
\|x\|_q^p\, g\!\left(\frac{x}{\|x\|_q}\right).
\]
This proves existence.

To prove uniqueness, suppose also that
\[
f(x) = \|x\|_q^p\, \tilde g\!\left(\frac{x}{\|x\|_q}\right)
\]
for some map \(\tilde g : \mathbb{S}^{D-1}_q \to \mathbb{R}^m\). Let
\(\theta \in \mathbb{S}^{D-1}_q\). Since \(\|\theta\|_q = 1\), evaluating both
...
g(\theta)=\tilde g(\theta)
\qquad
\text{for every } \theta \in \mathbb{S}^{D-1}_q.
\]
Since \(g\) and \(\tilde g\) have the same domain
\(\mathbb{S}^{D-1}_q\), they agree at every point of that domain, and hence
\(g=\tilde g\).
\end{proof}

\begin{remark}
The proposition shows that, for a given norm
\(\|\cdot\|_q\), a positively homogeneous map is completely determined by its
restriction to the \(q\)-sphere: the scaling behaviour is prescribed \(\|x\|_q^p\), and all remaining structure is represented by the
angular component \(g\).
\end{remark}

A corollary of \ref{prop:canonical-homogeneous-form} is that, on the subset
\[
\{\theta \in \mathbb{S}^{D-1}_q : g(\theta) \neq 0\},
\]
...
a(\theta) := \|g(\theta)\|_{q},
\qquad
e(\theta) := \frac{g(\theta)}{\|g(\theta)\|_{q}} \in \mathbb{S}^{m-1}.
\]
where $q$ need not be equal to $q$.
...
Under this representation, $a$ can be intuitively considered as an ``angle-dependent scaling factor'', $e$ an ``angle-map'' and $\|x\|_q^p$ a purely radial scaling term. 

[TODO: CHECK THIS, PRETTY SURE IT'S TRUE] More generally, the same argument applies whenever one replaces
\(\|x\|_q\) by a positive radial function \(r : \mathbb{O}_D \to (0,\infty)\)
satisfying
\[
...
\qquad x\in\mathbb{O}_D,
\]
for a unique angular map \(g:\mathbb{S}^{D-1}_q\to\mathbb{R}^m\). We therefore need only parameterise the angular component of the encoder.
...
where
\[
A_\phi:\mathbb{S}^{D-1}_q\to(0,\infty),
\qquad
E_\phi:\mathbb{S}^{D-1}_q\to\mathbb{R}^m.
\]
The softplus nonlinearity enforces positivity of \(a\), and normalisation enforces \(e(\theta)\in\mathbb{S}^{m-1}_{q}\). By construction, the resulting encoder is exactly positively homogeneous of degree \(p\). The importance of this formulation is that homogeneity of the encoder is not imposed by penalty or approximation but is ensured via the architectural setup. Then, by \Cref{prop:reg-variation-under-homogeneous-encoders} the latent representation
\[
Z=f(X)
\]
...
\]
the encoder scales ambient radius by the power \(p\), so the decoder inverts this through the factor \(\rho^{1/p}\). We first define the homogeneous decoder
\[
h_{\mathrm{hom}}(z)
=
\rho^{1/p}\,\tilde a(\eta)\,\tilde e(\eta), \qquad \tilde e:\mathbb{S}^{m-1}_{q}\to \mathbb{S}^{D-1}_{q}, \qquad \tilde a:\mathbb{S}^{m-1}_{q}\to (0,\infty),
\]
with angular map \(\tilde e\) and decoder-side magnitude term \(\tilde a\). This is the natural inverse analogue of the encoder parameterisation $f(x)=\|x\|_q^p\,a(\theta)\,e(\theta)$.
...
{\bigl\|\tilde e(\eta)+\delta(\eta,\rho)\bigr\|_q}
\in \mathbb{S}^{D-1}_{q},
\]
which is well-defined provided \(\|\tilde e(\eta)+\delta(\eta,\rho)\|_q\neq0\). 
...
If \(\delta\equiv 0\), then \(\tilde c(\eta,\rho)=\tilde e(\eta)\) for all \((\eta,\rho)\), and the decoder reduces to the positively homogeneous case \(h_{\mathrm{hom}}(z)\) of degree \(1/p\). More generally, the relevant asymptotic requirement is that the decoded direction become independent of latent radius in the tail [TODO: maybe formalise this a bit]:
\[
\tilde c(\eta,\rho)\to \tilde e(\eta)
\qquad\text{as }\rho\to\infty
\]
[in a suitable sense; e.g. pointwise or uniformly in \(\eta\)]. Note that this is equivalent to
\[
\delta(\eta,\rho)\to 0
\qquad\text{as }\rho\to\infty,
\]
since then the angular correction vanishes and the decoded direction becomes asymptotically homogeneous. [TODO: maybe formalise this a bit]
...
\qquad\text{as }\rho\to\infty.
\]
the angular correction vanishes in the radial limit, ensuring asymptotic homogeneity of the decoder. 
...
\qquad
\tilde e(\eta)=\frac{E_\psi(\eta)}{\|E_\psi(\eta)\|_q},
\qquad
...
\begin{remark}[Tail preservation under asymptotically homogeneous decoding]
[TODO: FORMALISE INTO THEOREM?] The key property of this decoder construction is that it preserves the tail behavior established by the encoder. Since the latent representation \(Z=f(X)\) is regularly varying with tail measure \(f_{\#}\mu\) (by \Cref{prop:reg-variation-under-homogeneous-encoders}), and the decoder converges to a homogeneous map \(h_{\mathrm{hom}}\) as \(\rho\to\infty\), the reconstructed variable \(\hat X = h(Z)\) should also be regularly varying with tail measure determined by the pushforward of \(f_{\#}\mu\) through \(h_{\mathrm{hom}}\)???. A rigorous statement might require showing that the asymptotic correction \(\delta(\eta,\rho)\to 0\) is sufficiently fast (uniformly in \(\eta\)) to ensure tail equivalence between \(h(Z)\) and \(h_{\mathrm{hom}}(Z)\). This might be supported by the compactness of \(\mathbb{S}^{m-1}_{q}\times[0,1]\) and continuity of \(\Delta_\psi\), which together imply uniform convergence of \(\delta\).
\end{remark}

\subsection{Training objective}
\label{sec:training-objective}

Let \(\hat X = h(f(X))\) denote the reconstruction of \(X\). The encoder and decoder are trained jointly using a reconstruction loss together with a radius-weighted penalty on the angular correction term to incentivise the decoder to revert to homogeneity where possible. Fix a norm
\[
\|\cdot\|_{\mathrm{pen}}
\]
on \(\mathbb{R}^D\), let
\[
\omega:\mathbb{R}^D\to(0,\infty)
\]
be a weight function, typically assigning (weakly) greater losses to deviations from homogeneity at larger radial scales. We then define the per-sample objective by
\[
\mathcal{L}(X)
=
...
\bigr\|_{\mathrm{pen}},
\]
where the argument to $\omega(\cdot)$ could also be a function of $X$ (such as $Z$ or $\hat{X}$), representing some notion of scale. The exact choice of $\omega$ is dependent upon the geometry of the manifold being modelled: if the manifold is highly curved at large radial scales, we want don't want $\omega$ to create a large loss when the network tries to (correctly) deviate from homogeneity. Alternatively, if the manifold being modelled is homogeneous from a relatively small radial distance onward, one would want $\omega$ to increase quickly to harshly punish (erroneous) deviations from homogeneity at larger radial scales. [TODO: rewrite this section in a more intuitive way - how much generality do I want to propose this in in the paper]. A simple choice of $\omega$ used in our implementation is $\omega=\log(1+\|.\|_q)$.[TODO: decide whether this should be the ambient radius, the latent radius, or another equivalent quantity].

Breaking down the loss, the first term denotes a typical reconstruction loss (e.g., $L_2$ norm) while the second penalises departures from the homogeneous angular decoder, potentially with a weight increasing in scale. The decoder is therefore encouraged to use the non-homogeneous term only where it materially improves reconstruction, while defaulting to the homogeneous mechanism whenever possible. The weight $\lambda_\mathrm{cor}$ dictates the balance between these two terms. 

This results in the intended adaptive asymptotic homogeneity. At moderate scales, the correction term can capture finite-scale curvature and other departures from homogeneous geometry. At large scales, the architectural condition \(\delta(\eta,\rho)\to 0\) suppresses the influence of the correction term, while the radius-weighted penalty discourages unnecessarily large corrections at finite scales. At large radial scales,
\[
\tilde c(\eta,\rho)\approx \tilde e(\eta),
\]
and the decoder is governed primarily by its homogeneous component. Reconstruction therefore remains compatible with the tail behaviour induced by the encoder.
