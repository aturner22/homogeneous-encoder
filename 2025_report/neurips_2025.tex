...
\text{in } \mathbb{M}(\mathbb{O}_D).
\]
The measure \(\mu\) is referred to as the \emph{ambient tail measure} of \(X\). By \cite[Theorem 3.1]{lindskog_regularly_2014}. Equivalently, the limit measure \(\mu\) is \(\alpha\)-homogeneous:
\[
...
To identify the tail (homogeneity) index of \(\mu_f\), let \(A\subset\mathbb{O}_m\) be measurable and bounded away from $0$ and let \(\lambda>0\). Using the homogeneity of \(f\),
\[
...
[TODO: THIS DECODER SECTION SHOULD BE AN ENTIRELY NEW SECTION]

This motivates the core architectural choice of this paper: structurally enforcing homogeneity of the encoder so that tail behaviour is transported to latent space in closed form. In contrast, the decoder need not be globally homogeneous and in fact, this ought to be avoided alltogether [TODO: MAYBE EXPLAIN THIS MORE USING DIAGRAMS OF HOW HOMOGENEOUS ENCODER CAN CAUSE OFF-MANIFOLD RECONSTRUCTIONS]. Exact homogeneity is useful for tail transport, but in fact structurally incompatible for reconstruction of curved manifold geometry.

In order to tackle this problem, we model the decoder as
\[
h(z)=h_{\mathrm{hom}}(z)+h_{\mathrm{cor}}(z),
\]
where \(h_{\mathrm{hom}}\) is homogeneous and \(h_{\mathrm{cor}}\) is a correction to capture non-homogeneous (curved) features of \(\cM\) at moderate scales, while a regularisation penalty (see section [TODO: REF THIS SECTION] for details) forces \(h_{\mathrm{cor}}\) to be negligible relative to \(h_{\mathrm{hom}}\) in the tail. At large scales, decoding is controlled by the homogeneous component, so the asymptotic properties of the latent representation are retained upon reconstruction. [TODO: REWRITE THIS SECTION TO EXPLAIN WHY ASYMPTOTIC HOMOGENEITY IS COMPATIBLE WITH THE AMBIENT MANIFOLD BY ASSUMPTION]. This is the basis of the presented autoencoder framework: latent representations are learned in a way that is compatible with regular variation, while complex ambient manifold geometry can be recovered through an adaptively asymptotically homogeneous decoder.

\subsection{General form of homogeneous maps}
\label{sec:homogeneous-maps}
TODO: CHECK THIS SECTION: 

In this section we show that every homogeneous map from Euclidian space admits a canonical radial--angular representation. This is extremely useful for learning, since homogeneity is enforced by construction, one may parameterise the angular component unrestricted while preserving the required scaling law automatically. This motivates the architecture of the encoder architecture used later in section [TODO: REF SECTION].

In this section we record a basic structural fact about positively homogeneous maps on
\[
\mathbb{O}_D:=\mathbb{R}^D\setminus\{0\}.
\]
Writing \(x\in\mathbb{O}_D\) in polar form,
\[
x=\rho\,\theta,
\qquad
\rho:=\|x\|_q>0,
...
\]
one sees that positive homogeneity is equivalent to a separation of radial and angular variables. This is useful for parameterisation: the radial dependence is fixed by the homogeneity degree, while the remaining freedom is carried entirely by an unconstrained angular map. This observation underlies the encoder parameterisation used later in Section~\ref{...}.

\begin{proposition}[Product form of positively homogeneous maps]
\label{prop:canonical-homogeneous-form}
Let \(f:\mathbb{O}_D\to\mathbb{R}^m\) and let \(p>0\). Then the following are equivalent:
\begin{enumerate}
    \item \(f\) is positively homogeneous of degree \(p\), i.e.
    \[
    f(\lambda x)=\lambda^p f(x),
...
\begin{proof}
Assume first that \(f\) is positively homogeneous of degree \(p\). Define
\[
g(\theta):=f(\theta),
\qquad \theta\in\mathbb{S}^{D-1}_q.
\]
For any \(x\in\mathbb{O}_D\), writing
\[
x=\|x\|_q\,\frac{x}{\|x\|_q},
\]
the homogeneity of \(f\) gives
\[
f(x)
=
...
\]
for some \(\tilde g:\mathbb{S}^{D-1}_q\to\mathbb{R}^m\). Evaluating at \(x=\theta\in\mathbb{S}^{D-1}_q\), for which \(\|\theta\|_q=1\), yields
\[
g(\theta)=f(\theta)=\tilde g(\theta),
\]
so \(g=\tilde g\).
...
\lambda^p f(x).
\]
Thus \(f\) is positively homogeneous of degree \(p\).
\end{proof}

\begin{remark}
The proposition shows that a positively homogeneous map is determined completely by its restriction to the unit \(q\)-sphere. The radial dependence is prescribed by the power law \(\|x\|_q^p\), and all remaining structure is encoded by the angular component \(g\).
\end{remark}

When \(g(\theta)\neq 0\), one may further decompose
\[
g(\theta)=a(\theta)e(\theta),
\qquad
a(\theta):=\|g(\theta)\|_2,
\qquad
e(\theta):=\frac{g(\theta)}{\|g(\theta)\|_2}\in\mathbb{S}^{m-1}.
\]
Accordingly,
\[
f(x)=\|x\|_q^p\,a\!\left(\frac{x}{\|x\|_q}\right)e\!\left(\frac{x}{\|x\|_q}\right).
\]
The same argument applies verbatim to any positively homogeneous choice of radial coordinate and associated angular normalisation.
...
\qquad
E_{\mathrm{mag}}:\mathbb{S}^{D-1}_{q_\pi}\to\RR.
\]
...
+
c_{\mathrm{cor}}(\theta(z),\log \rho(z))
\Bigr).
\]
...
\qquad
c_{\mathrm{cor}}(\theta,\log\rho)=H_{\mathrm{cor}}(\theta,\log\rho),
\]
with
\[
...
\lambda_{\mathrm{cor}}\,
\rho(f(x))\,
\norm{c_{\mathrm{cor}}(\theta(f(x)),\log \rho(f(x)))}_1.
\]
...
\begin{enumerate}
    ...
    \item \(c_{\text{correction}}(\theta, \log r_w): \mathbb{S}^{m-1} \times \RR \to \RR^D\) --- sparse non-homogeneous correction
...
c_correction = MLP_correction([theta, log(r_w)])  (non-homogeneous)
